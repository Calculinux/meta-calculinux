SUMMARY = "Default 6x12 cruft console font (Terminus + Fairfax)"
DESCRIPTION = "Native 6x12 bitmap merge for the cruft framebuffer console: Terminus \
...
Demand-paged CRUFTFN1 mmap blob for the Luckfox Lyra RAM budget."
...
assert (cw, ch, gs, ps) == (6, 12, 32, 8), (cw, ch, gs, ps)
size = os.path.getsize(path)
assert size > 200000, size
print("console.cruftfont ok:", cw, "x", ch, size, "bytes")
PY
}

do_install() {
    install -d ${D}${datadir}/cruft
    install -m 0644 ${B}/console.cruftfont ${D}${datadir}/cruft/console.cruftfont
}

FILES:${PN} = "${datadir}/cruft/console.cruftfont"
